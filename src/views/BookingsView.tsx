...
<body>
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden print-area">
      ...
      <div className="p-4 md:p-6 border-b border-slate-100 bg-white space-y-3 no-print">
        <div className="flex items-start md:items-center justify-between gap-3 flex-col sm:flex-row">
          <div className="min-w-0">
            ...
            <p className="text-base md:text-lg font-bold text-slate-800 leading-tight mt-1">
              Rekod Penggunaan Sumber
            </p>
          </div>
          <div className="flex gap-1.5 items-center w-full sm:w-auto">
            <span className="text-[10px] font-bold px-2.5 py-1.5 bg-blue-50 text-blue-700 rounded-lg uppercase tracking-widest flex-1 sm:flex-none text-center">
      ...
      <div className="md:hidden divide-y divide-slate-100">
        {filtered.length > 0 ? filtered.map((b) => {
          ...
          const borderTone =
            ...
            b.status === 'cancelled' ? 'border-l-rose-500' :
            b.status === 'returned'  ? 'border-l-emerald-500' :
            'border-l-amber-500';
          return (
            <div key={b.id} className={`p-4 border-l-4 ${borderTone} bg-white`}>
              <div className="flex items-start justify-between gap-2 mb-2">
                <div className="min-w-0 flex-1">
                  <p className="text-[10px] font-mono text-blue-600 uppercase">
                    #{b.id.split('-')[1]?.slice(-6) ?? b.id.slice(-6)}
                  </p>
                  <p className="text-sm font-bold text-slate-800 leading-snug truncate">{resourceName}</p>
                </div>
                <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded border ${statusTone} shrink-0`}>
              ...
              <div className="grid grid-cols-2 gap-2 text-[11px] mb-2">
                <div className="flex items-center gap-1.5 text-slate-700">
                  ...
                  <span className="font-bold truncate">{b.date}</span>
                </div>
                <div className="flex items-center gap-1.5 text-slate-700">
              ...
              <div className="flex items-center gap-2 mb-2">
                <div className="w-6 h-6 rounded-full bg-slate-100 flex items-center justify-center text-[9px] font-bold text-slate-600 border border-slate-200 shrink-0">
                  {initials}
                </div>
                <span className="text-[11px] font-semibold text-slate-700 truncate">{b.userName}</span>
              </div>

              <p className="text-[11px] text-slate-600 leading-relaxed line-clamp-2 mb-2">{b.purpose}</p>
              ...
              {b.status === 'returned' && b.returnedAt && (
                <p className="text-[10px] text-emerald-700 mb-2">
                  ...
                  {b.returnedByName && <span className="text-slate-400"> · {b.returnedByName}</span>}
                </p>
              )}
          ...
          <div className="px-6 py-12 text-center text-slate-400 font-medium text-sm italic">
